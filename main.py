...
def organize():
    ...
    for file in files: # Percorre todos os arquivos dentro do caminho selecionado
        file_name, file_extension = os.path.splitext(file) # Separa os arquivos em 2(duas) partes: nome_do_arquivo, extensao_do_arquivo
        
        if file_extension != '': # Pastas, por padrão, não possuem extensão, assim você consegue separar as pastas criadas (Evita do programa tentar mover uma das pastas de organização para dentro dela mesma)
            if (path / Path(file_extension.upper())).exists(): # Checa se no caminho selecionado existe uma pasta com o nome da extensão da mesma, se retornar True:
                shutil.move(file, Path(path / file_extension.upper())) # Move o arquivo para dentro da pasta que possue o nome de sua extensão
            ...
            if not (path / Path(file_extension.upper())).exists(): # Caso não possua uma pasta com o nome da extensão:
                ...
                shutil.move(file, Path(path / file_extension.upper())) # Move o arquivo para a pasta que possue o nome de sua extensão
...
# Items
text_label = Label(root, text='Click there to select the file -->', bg='#969696') # Label: Utilizada para exibir textos.

path_button = Button(root, text='There', command=get_path, width=20, bg='#969696') # Button: Botão que executa a função get_path
organize_button = Button(root, text='Organizer', command=organize, bg='#969696') # Button: Botão que executa a função organize
